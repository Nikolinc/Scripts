...
if os.path.exists("/etc/os-release"):
    for line in open("/etc/os-release"):
        if line.startswith("ID="):
            ...
            break
if os_id in ["ubuntu","debian"]: pm="apt"
elif os_id in ["centos","rhel","fedora"]: pm="dnf" if os_id=="fedora" else "yum"
elif os_id in ["arch"]: pm="pacman"

# Обновление
if pm=="apt": run(f"{pm} update -y && {pm} upgrade -y")
elif pm in ["yum","dnf"]: run(f"{pm} -y update")
elif pm=="pacman": run("pacman -Syu --noconfirm")
